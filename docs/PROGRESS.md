...
**테스트**: 113 passing (blackbox + whitebox + 파서/임포트 실파일 검증), `tsc --noEmit` clean, `electron-vite build` 성공. 스키마 v2(결제일 수동 플래그 + updated_at) 마이그레이션 + 업그레이드/백업 테스트 포함. (2026-07-08 클린 `npm ci` 재검증: typecheck/test/build 3종 그린.)
...
## 다음
1. **.exe 인계 = GitHub Release로 배포 중** — 최신 `v0.1.2`가 Release에 첨부됨(포터블 `DueLedger_v0.1.2.exe` · 설치본 `DueLedger_Setup_v0.1.2.exe` · `DueLedger_v0.1.2_win.zip`, https://github.com/dlrhksdn25199/dueledger/releases/tag/v0.1.2). 그 1명에게 Release 링크 전달하면 바로 다운로드. 미서명이라 첫 실행 시 "추가 정보 → 실행" 1회 안내. 다음 버전도 `npm version` → `package:win` → `gh release create vX.Y.Z release/DueLedger_v*.exe` 흐름. productName/회사명 확정 시 `electron-builder.yml` 갱신.
