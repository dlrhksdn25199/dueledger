...
## 현재 상태 (2026-06-16)
**계층**: domain + repository + **Electron/React UI**(홈 대시보드·달력·상태배지 포함) + **패키징(electron-builder)** 완료.
**테스트**: 79 passing (blackbox + whitebox), `tsc --noEmit` clean, `electron-vite build` 성공.
**맥 실기기 검증**: CDP 스모크(`scripts/cdp-smoke.mjs`) **패키지된 앱 17/17 통과** — 시드·거래처·명세서 입력·금액(12,345→부가세 1,235→합계 13,580)·결제일(net-30→2026-07-16)·정렬·사용중 삭제 차단 + **홈 카드(예정 1건/총 미지급 13,580)·결제상태 배지·달력 7/16 결제 표시**. 프로덕션 경로(loadFile + asar 언팩 better-sqlite3 + userData DB)까지 → .exe 동일 동작 근거.
