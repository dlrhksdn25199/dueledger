...
## 다음
1. **.exe 인계 = GitHub Release로 배포 중** — 최신 `v0.1.1`이 Release에 첨부됨(https://github.com/dlrhksdn25199/dueledger/releases). 그 1명에게 Release 링크 전달하면 바로 다운로드. 미서명이라 첫 실행 시 "추가 정보 → 실행" 1회 안내. 다음 버전도 `npm version` → `package:win` → `gh release create vX.Y.Z release/DueLedger_v*.exe` 흐름. productName/회사명 확정 시 `electron-builder.yml` 갱신.
