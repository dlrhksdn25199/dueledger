...
## 다음
1. **.exe 인계** — CI 아티팩트(`DueLedger-*-portable.exe`) 다운로드 → 그 1명에게 전달. 미서명이라 첫 실행 시 "추가 정보 → 실행" 1회 안내. 아이콘=`docs/dueledger-icon.ico` 적용됨. productName/회사명 확정 시 `electron-builder.yml` 갱신.
