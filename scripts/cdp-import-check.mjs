// 임포트 라이브 검증 — 실행 중 앱에서 window.api.import.preview/commit를 실제 샘플 파일로 호출.
...
const SAMPLE = 'C:/Users/ltim2/source/repos/dueledger/docs/거래명세서_공용.xlsx';
...
check('preview 신규 품목 36', pv?.newItems === 36, JSON.stringify({ st: pv?.newStatements, items: pv?.newItems, dup: pv?.duplicateItems }));
...
check('commit 후 품목 36 적재', c1?.newItems === 36, JSON.stringify({ st: c1?.newStatements, items: c1?.newItems }));
const ledgerRows = await evalJS(`window.api.ledger.list().then(r=>r.length)`);
check('원장에 36줄 보임', ledgerRows === 36, `rows=${ledgerRows}`);
...
check('거래처 자동 생성됨', vendors > 0, `vendors=${vendors}`);
check('카테고리에 부재료/부자재 자동 생성', cats.includes('부재료') && cats.includes('부자재'), cats.join(','));

// 4) 재임포트 — 전부 중복
const c2 = await evalJS(`window.api.import.commit(${JSON.stringify(SAMPLE)})`);
check('재임포트는 전부 중복(신규 0)', c2?.newItems === 0 && c2?.duplicateItems === 36, JSON.stringify({ items: c2?.newItems, dup: c2?.duplicateItems }));
const ledgerRows2 = await evalJS(`window.api.ledger.list().then(r=>r.length)`);
check('재임포트 후에도 36줄 (2배 안 됨)', ledgerRows2 === 36, `rows=${ledgerRows2}`);
