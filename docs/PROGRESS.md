...
**⚠️ 노후화된 `cdp-smoke.mjs`**: `'카테고리'` 탭을 클릭하나 UI는 `'거래처·카테고리'`로 병합됨(스모크 마지막 수정 6/17 12:19 < 탭 병합 6/17 14:16). 회귀로 다시 쓰려면 탭명·`table.grid` 선택자 갱신 필요. (v0.1.3 검증은 `cdp-features.mjs`로 대체함.)
...
3. (선택·나중) 세율 변경 시 기존 명세서 vat 소급 재계산(현재 비소급) · `cdp-smoke.mjs` 탭병합 반영 수리 · `npm ci` 후 electron 바이너리 자동 설치 보장.
